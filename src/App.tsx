      ...
      <header className="bg-emerald-600 text-white py-6 px-4 shadow-lg sticky top-0 z-10">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          >
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">アグリ・アドバイザー</h1>
              <p className="text-emerald-100 text-sm">アグリさんがあなたの農業をサポート！</p>
            </div>
          </motion.div>
          <button 
            onClick={reset}
        ...
        >
          <div className="relative flex-shrink-0">
            <div className="w-32 h-32 rounded-full border-4 border-emerald-500 overflow-hidden bg-emerald-100 shadow-inner">
              <img 
                src="https://picsum.photos/seed/agri-san-pear-orchard/300/300" 
                alt="Agri-san" 
          ...
          <div className="text-center md:text-left">
            <h2 className="text-xl font-bold mb-2 text-emerald-800">「お疲れ様！アグリだよ。」</h2>
            ...
            <div className="space-y-4">
              <div>
                <label className="flex items-center justify-between mb-2 text-sm font-medium">
                  ...
                  <span className="text-emerald-600 font-bold">{sensorData.moisture}%</span>
                </label>
                <input 
                  type="range" 
                  min="0" 
                  ...
                  onChange={(e) => setSensorData({...sensorData, moisture: parseInt(e.target.value)})}
              ...
              <div>
                <label className="flex items-center justify-between mb-2 text-sm font-medium">
                  <span className="flex items-center gap-2">
                    <div className="w-4 h-4 rounded-full bg-purple-500 flex items-center justify-center text-[10px] text-white">pH</div>
                    pH (酸性度)
                  </span>
                  <span className="text-emerald-600 font-bold">{sensorData.ph}</span>
                </label>
                <input 
                  type="range" 
                  min="0" 
                  ...
                  step="0.1"
                  value={sensorData.ph}
                  onChange={(e) => setSensorData({...sensorData, ph: parseFloat(e.target.value)})}
                  className="w-full h-2 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-emerald-500"
                />
              </div>
              ...
              <div>
                <label className="flex items-center justify-between mb-2 text-sm font-medium">
                  ...
                  <span className="text-emerald-600 font-bold">{sensorData.ec} mS/cm</span>
                </label>
                <input 
                  type="range" 
                  min="0" 
                  ...
                  step="0.1"
                  value={sensorData.ec}
                  onChange={(e) => setSensorData({...sensorData, ec: parseFloat(e.target.value)})}
                  className="w-full h-2 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-emerald-500"
                />
              </div>
            </div>
          ...
          >
            ...
            <div 
              ...
              className={`flex-1 border-2 border-dashed rounded-xl flex flex-col items-center justify-center p-4 cursor-pointer transition-all min-h-[160px] ${
                image ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 hover:border-emerald-300 hover:bg-slate-50'
              }`}
            >
              {image ? (
                ...
                <div className="text-center space-y-2">
                  ...
                  <p className="text-slate-500 text-sm">クリックして写真を選択</p>
                  <p className="text-slate-400 text-xs">（葉の状態がわかる写真がおすすめ）</p>
                </div>
              )}
              <input 
                type="file" 
            ...
            {/* Sample Images */}
            <div className="mt-4">
              <p className="text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">サンプル写真で試す</p>
              <div className="flex gap-2">
                {[
                  { id: 'healthy', url: 'https://picsum.photos/seed/green-leaf/400/300', label: '元気な葉' },
                  { id: 'dry', url: 'https://picsum.photos/seed/dry-plant/400/300', label: '乾燥気味' },
                  { id: 'yellow', url: 'https://picsum.photos/seed/yellow-leaf/400/300', label: '変色あり' }
                ].map((sample) => (
                  <button
                    key={sample.id}
                    onClick={(e) => {
                  ...
                  >
                    <img src={sample.url} alt={sample.label} className="w-full h-full object-cover group-hover:scale-110 transition-transform" referrerPolicy="no-referrer" />
                    <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <span className="text-[10px] text-white font-bold">{sample.label}</span>
                    </div>
                  </button>
                ))}
              </div>
            ...
            >
              ...
              <div className="flex items-center justify-between pb-4 border-b border-slate-100 relative z-10">
                ...
                <button
                  ...
                  className={`p-3 rounded-full transition-all ${
                    isSpeaking ? 'bg-emerald-100 text-emerald-600 animate-pulse' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100'
                  }`}
                  title="アドバイスを聴く"
                >
                  {isSpeaking ? <Volume2 className="w-6 h-6" /> : <Volume2 className="w-6 h-6" />}
                </button>
              </div>
              ...
              <div className="pt-6 border-t border-slate-100 flex flex-wrap gap-4 relative z-10">
                ...
                <div className="bg-purple-50 px-4 py-2 rounded-full text-sm font-medium text-purple-700 flex items-center gap-2">
                  <span className="text-[10px] font-bold">pH</span> {sensorData.ph}
                </div>
              ...
              {audioUrl && (
                <audio ref={audioRef} src={audioUrl} className="hidden" onEnded={() => setIsSpeaking(false)} />
              )}
            </motion.section>
          )}
        </AnimatePresence>
